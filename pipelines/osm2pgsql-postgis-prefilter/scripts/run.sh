...
PIPELINE_ID="osm2pgsql-postgis-prefilter"
...
echo "[pipeline-b2] start postgres cluster"
pg_ctlcluster 16 main start
trap 'pg_ctlcluster 16 main stop' EXIT

echo "[pipeline-b2] create benchmark database"
T2=$(date +%s%3N)
...
echo "[pipeline-b2] import filtered pbf with osm2pgsql flex"
T4=$(date +%s%3N)
...
echo "[pipeline-b2] run postprocessing SQL"
T6=$(date +%s%3N)
...
echo "[pipeline-b2] export ndjson from PostGIS"
T8=$(date +%s%3N)
...
echo "[pipeline-b2] build pmtiles"
T12=$(date +%s%3N)
...
echo "[pipeline-b2] run validations"
T14=$(date +%s%3N)
...
validation = {
    "pipeline": "osm2pgsql-postgis-prefilter",
